...
import { attribute, isDiagram, isRelation, isView, metaClass } from './adapter.js';
import type { Diagram, Issue, ModelElement, ParsedModel, Relation, View } from './types.js';

type RawNode = { tag: string; attrs: Record<string, string>; path: string; parentId?: string; diagramId?: string };
const idNames = ['xmi:id', 'xmi:idref', 'id', 'ID'];
...
export function parseSysmlXml(xml: string, fileName = 'model.xml'): ParsedModel {
  ...
  const visit = (nodes: unknown[], path: string, parentId?: string, diagramId?: string): void => {
    for (const item of nodes) {
      ...
      for (const [tag, value] of Object.entries(item as Record<string, unknown>)) {
        ...
        const id = attribute(attrs, ...idNames) ?? `generated-${++generated}`;
        const type = metaClass(tag, attrs);
        const node: RawNode = { tag, attrs, path: nodePath, parentId, diagramId };
        ...
        } else if (attribute(attrs, ...idNames) || ['Model', 'Package'].includes(type)) {
        ...
        raw.push(node);
        const child = Array.isArray(value) ? value : []; visit(child, nodePath, id, currentDiagramId);
      }
    }
  };
  visit(doc, '');
  ...
  for (const obj of allObjects.values()) {
    ...
    if (!obj.name && obj.metaClass !== 'Model') issues.push({ code: 'MISSING_NAME', severity: 'warning', message: `${obj.metaClass} has no name`, xpath: obj.sourceXPath, elementId: obj.id });
  }
  ...
  return { id: crypto.randomUUID(), source: { fileName, encoding: /^\s*<\?xml[^>]*encoding=["']([^"']+)/i.exec(xml)?.[1] ?? 'UTF-8', xmiVersion: attribute(rootAttrs, 'xmi:version', 'xmiVersion'), productVersion: attribute(rootAttrs, 'productVersion') }, elements: [...elements.values()], relations: [...relations.values()], diagrams: [...diagrams.values()], views: [...views.values()], issues, statistics: { elements: elements.size, relations: relations.size, diagrams: diagrams.size, views: views.size, danglingReferences: issues.filter(x => x.code === 'DANGLING_REFERENCE').length, duplicateIds: duplicateIds.size } };
}
