...
export function isRelation(type: string, tag: string): boolean {
  return RELATION_TYPES.has(type) || RELATION_TYPES.has(localName(tag));
}
export function isDiagram(type: string, tag: string, attrs: Record<string, string>): boolean {
  return DIAGRAM_TYPES.has(type) || DIAGRAM_TYPES.has(localName(tag)) || Boolean(attribute(attrs, 'diagramType', 'humanType'));
}
